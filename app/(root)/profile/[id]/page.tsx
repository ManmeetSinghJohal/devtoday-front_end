...
const Page = async ({
  ...
  const resUser = await fetch(
    `${process.env.SERVER_URL}/api/user/${params?.id}?viewerId=${session?.user.id}`,
    {
      method: "GET",
      mode: "cors",
  ...
  if (type !== "group") {
    resPosts = await fetch(
      `${process.env.SERVER_URL}/api/user/${params?.id}/posts?postType=${type.toUpperCase()}`,
      {
        method: "GET",
        mode: "cors",
        headers: { "Content-Type": "application/json" },
    ...
    resPosts = await fetch(
      `${process.env.SERVER_URL}/api/user/${params?.id}/groups`,
      {
        method: "GET",
        mode: "cors",
        headers: { "Content-Type": "application/json" },
